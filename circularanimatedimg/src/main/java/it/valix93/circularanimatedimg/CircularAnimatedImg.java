...
public class CircularAnimatedImg extends android.support.v7.widget.AppCompatImageView {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray attributes = context.obtainStyledAttributes(attrs, R.styleable.CircularAnimatedImg, defStyleAttr, 0);

        // Init Wave

        waveColor = -16728876;
        float amplitudeRatioAttr = attributes.getFloat(R.styleable.CircularAnimatedImg_cai_wave_amplitude, DEFAULT_AMPLITUDE_RATIO);
        amplitudeRatio = (amplitudeRatioAttr > DEFAULT_AMPLITUDE_RATIO) ? DEFAULT_AMPLITUDE_RATIO : amplitudeRatioAttr;
        setProgress(attributes.getInteger(R.styleable.CircularAnimatedImg_cai_progress, 0));

        if (attributes.getBoolean(R.styleable.CircularAnimatedImg_cai_border, true)) {
            float defaultBorderSize = DEFAULT_BORDER_WIDTH * getContext().getResources().getDisplayMetrics().density;
            borderPaint.setStrokeWidth(attributes.getDimension(R.styleable.CircularAnimatedImg_cai_border_width, defaultBorderSize));
        } else {
